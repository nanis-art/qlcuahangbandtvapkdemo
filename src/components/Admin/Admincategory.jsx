...
function AdminCategory({ embedded = false }) {
  ...
  const displayedRows = useMemo(() => {
    ...
    if (!q) return rows;
    return rows.filter((r) => String(r.id ?? r.idcategory) === q);
  ...
  const handleSubmitForm = (e) => {
    ...
    const nextId = rows.reduce((m, r) => Math.max(m, Number(r.id ?? r.idcategory) || 0), 0) + 1;
    const built = formToRow(form, nextId);
    let nextList;
    if (isNew) {
      nextList = [...rows, built];
    } else {
      const idx = rows.findIndex((r) => String(r.id ?? r.idcategory) === String(form.id));
      if (idx === -1) {
        setSaveError('Không tìm thấy bản ghi để cập nhật');
        return;
      }
      nextList = rows.map((r) => (String(r.id ?? r.idcategory) === String(form.id) ? built : r));
    }
    persist(nextList);
  };
  ...
  const handleDelete = (id) => {
    ...
    persist(rows.filter((r) => String(r.id ?? r.idcategory) !== String(id)));
  };
              ...
              <table className="admin-table">
                ...
                <tbody>
                  {displayedRows.length === 0 ? (
                    ...
                    displayedRows.map((r) => (
                      <tr key={r.id ?? r.idcategory}>
                        <td>{r.id ?? r.idcategory}</td>
                        <td>{r.name}</td>
                        <td>
                          <div className="admin-table_actions">
                            ...
                            <button type="button" className="admin-table_link admin-table_link--danger" onClick={() => handleDelete(r.id ?? r.idcategory)} disabled={saving}>
                              Xóa
                            </button>
                          </div>
                        </td>
