...
function AdminEmployee({ embedded = false }) {
  ...
  const handleSubmitForm = (e) => {
    ...
    const nextId = rows.reduce((m, r) => Math.max(m, Number(r.id ?? r.idemployee) || 0), 0) + 1;
    const built = formToRow(form, nextId);
    let nextList;
    if (isNew) {
      nextList = [...rows, built];
    } else {
      const idx = rows.findIndex((r) => String(r.id ?? r.idemployee) === String(form.id));
      if (idx === -1) {
        setSaveError('Không tìm thấy bản ghi để cập nhật');
        return;
      }
      nextList = rows.map((r) => (String(r.id ?? r.idemployee) === String(form.id) ? built : r));
    }
    persist(nextList);
  };
                      ...
                      <tr key={r.id ?? r.idemployee}>
                        ...
                        <td>
                          <div className="admin-table_actions">
                            ...
                            <button type="button" className="admin-table_link admin-table_link--danger" onClick={() => handleDelete(r.id ?? r.idemployee)} disabled={saving}>
                              Xóa
                            </button>
                          </div>
                        </td>
