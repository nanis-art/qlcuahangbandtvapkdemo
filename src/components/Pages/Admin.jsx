    ...
    <div className="ruang-layout">
      ...
      <div className="ruang-shell">
        ...
        <footer className="ruang-footer">
          ...
          {/* copyright Quản lý bán điện thoại giao diện theo phong cách{" "}
      ...
      {logoutModalOpen && (
        <div className="ruang-modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="ruang-logout-title">
          <div className="ruang-modal">
            <div className="ruang-modal_header">
              <h5 id="ruang-logout-title">Ohh No!</h5>
              <button type="button" className="ruang-modal_close" onClick={() => setLogoutModalOpen(false)} aria-label="Đóng">
                X
              </button>
            </div>
...
//   // 2. LỌC TOP SẢN PHẨM BÁN CHẠY (Chống lỗi NaN)
//   const topSoldProducts = useMemo(() => {
//     const byProduct = invoiceDetails.reduce((map, item) => {
//       const pid = Number(item.product_id);
//       if (isNaN(pid)) return map; // Bỏ qua sản phẩm lỗi
//       const quantity = Number(item.quantity || 0);
//       map.set(pid, (map.get(pid) || 0) + quantity);
//       return map;
//     }, new Map());
...
//           id,
//           sold,
//           name: product?.name || `Sản phẩm ẩn #${id}`,
//         };
//       })
//       .sort((a, b) => b.sold - a.sold)
//       .slice(0, 5)
...
//   // 3. LỌC DOANH THU THEO NGÀY (Chống lỗi ngày undefined)
//   const revenueByDate = useMemo(() => {
//     const grouped = bills.reduce((acc, bill) => {
//       const key = String(bill.date || "").slice(0, 10);
//       if (!key || key === "undefined" || key === "null") return acc; // Bỏ qua ngày lỗi
//       acc.set(key, (acc.get(key) || 0) + Number(bill.total || 0));
//       return acc;
//     }, new Map());
...
//   // 4. LỌC BẢNG HÓA ĐƠN (Chống khách lẻ, sản phẩm ẩn)
//   const billTableRows = useMemo(() => {
//     const customerMap = new Map(customers.map(c => [Number(c.id), c.name]));
//     const productMap = new Map(products.map(p => [Number(p.id), p.name]));
//     const detailByBill = invoiceDetails.reduce((map, item) => {
...
//         // Fix lỗi hiển thị
//         const cid = Number(bill.customer_id);
//         const customerName = (!isNaN(cid) && customerMap.get(cid)) ? customerMap.get(cid) : "Khách lẻ";
//         const itemName = firstProduct ? (productMap.get(Number(firstProduct.product_id)) || "Sản phẩm ẩn") : "Trống";
...
//   // 5. LỌC KHÁCH HÀNG VIP (Chống lỗi khách ảo)
//   const vipCustomers = useMemo(() => {
//     if (!bills.length) return [];
...
//     const latestDate = validBills.map(b => String(b.date)).sort().slice(-1)[0];
//     const targetMonth = latestDate.slice(0, 7);
//     const customerMap = new Map(customers.map(c => [Number(c.id), c.name]));
...
//       const cid = Number(bill.customer_id);
//       if (isNaN(cid)) return map; // Bỏ qua khách lẻ
...
//                   <div className="ruang-user_menu-title">Tài khoản</div>
//                   <button type="button" role="menuitem" onClick={goHome}>
//                     <i className="bi bi-house-door" /> Trang chủ Web
//                   </button>
//                   <button
//                     type="button"
//                     role="menuitem"
...
//           {/* CÁC ROUTE QUẢN LÝ (Sau này code thêm component thì mở khóa) */}
//           {adminSection === "products" ? <div style={{padding:"20px", fontWeight:"600"}}>Giao diện Quản lý Sản phẩm</div> : null}
//           {adminSection === "category" ? <div style={{padding:"20px", fontWeight:"600"}}>Giao diện Quản lý Danh mục</div> : null}
//           {adminSection === "customer" ? <div style={{padding:"20px", fontWeight:"600"}}>Giao diện Quản lý Khách hàng</div> : null}
//           {adminSection === "employee" ? <div style={{padding:"20px", fontWeight:"600"}}>Giao diện Quản lý Nhân viên</div> : null}
//           {adminSection === "bill" ? <div style={{padding:"20px", fontWeight:"600"}}>Giao diện Quản lý Hóa đơn</div> : null}
//           {adminSection === "invoiceDetails" ? <div style={{padding:"20px", fontWeight:"600"}}>Giao diện Chi tiết Hóa đơn</div> : null}

//           {/* DASHBOARD CHÍNH */}
//           {adminSection === "dashboard" && (
//             loading ? <div className="ruang-loading">Đang tải dữ liệu hệ thống...</div> : (
//             <>
//               <div className="ruang-cards">
//                 <div className="ruang-stat-card">
//                   <div className="ruang-stat-card_body">
...
//                 <div className="ruang-stat-card ruang-stat-card--green">
//                   <div className="ruang-stat-card_body">
//                     <div className="ruang-stat-card_label">SP Đã bán</div>
//                     <div className="ruang-stat-card_value">{fmtNumber(stats.soldSum)}</div>
//                     <div className="ruang-stat-card_badge">Toàn thời gian</div>
//                   </div>
//                   <div className="ruang-stat-card_icon" aria-hidden>
//                     <i className="bi bi-cart3" />
//                   </div>
...
//                   </div>
//                   <div className="ruang-revenue-list">
//                     {revenueByDate.length === 0 ? <p style={{color:'#64748b', fontSize:'0.85rem'}}>Chưa có dữ liệu giao dịch hợp lệ.</p> :
//                      revenueByDate.map(row => (
//                       <div className="ruang-revenue-item" key={row.date}>
//                         <div className="ruang-revenue-item_head">
//                           <span>{row.date}</span>
...
//                   </div>
//                   <div className="ruang-sold-list">
//                     {topSoldProducts.length === 0 ? <p style={{color:'#64748b', fontSize:'0.85rem'}}>Chưa có sản phẩm nào được bán ra.</p> :
//                      topSoldProducts.map((item, idx) => (
//                       <div className="ruang-sold-item" key={item.id}>
//                         <div className="ruang-sold-item_head">
//                           <span>{item.name}</span>
//                           <strong>Đã bán: {item.sold}</strong>
//                         </div>
//                         <div className="ruang-sold-item_bar">
//                           <div className={`ruang-sold-item_fill ruang-sold-item_fill--${idx % 4}`} style={{ width: `${item.percent}%` }} />
//                         </div>
...
//                     <button type="button" className="ruang-mini-btn" onClick={() => setAdminSection("bill")}>
...
//                           <tr><td colSpan="4" className="admin-table_empty">Chưa có đơn hàng nào trong hệ thống.</td></tr>
//                         ) : billTableRows.map(row => (
//                           <tr key={row.id}>
//                             <td>#{row.billCode}</td>
//                             <td>{row.customerName}</td>
...
//                 </div>
//                 <div className="ruang-card">
//                   <div className="ruang-vip-title">Khách hàng VIP</div>
//                   <div className="ruang-vip-list">
//                     {vipCustomers.length === 0 ? <div className="ruang-vip-item"><small style={{color:'#64748b'}}>Chưa có dữ liệu khách hàng VIP.</small></div> :
//                      vipCustomers.map(vip => (
//                       <div key={vip.customerId} className="ruang-vip-item">
//                         <strong>{vip.name}</strong>
//                         <small>{vip.count} đơn hàng — {fmtCurrency(vip.total)}</small>
//                       </div>
//                     ))}
//                   </div>
//                 </div>
...
//           <div className="ruang-modal">
//             <div className="ruang-modal_header">
//               <h5 id="ruang-logout-title">Xác nhận</h5>
//               <button type="button" className="ruang-modal_close" onClick={() => setLogoutModalOpen(false)} aria-label="Đóng">
...
//             <div className="ruang-modal_footer">
//               <button type="button" className="admin-btn admin-btn--ghost" onClick={() => setLogoutModalOpen(false)}>
//                 Hủy
//               </button>
//               <button type="button" className="admin-btn" style={{background:'#ef4444', borderColor:'#ef4444'}} onClick={logout}>
//                 Đăng xuất
//               </button>
//             </div>
//           </div>
