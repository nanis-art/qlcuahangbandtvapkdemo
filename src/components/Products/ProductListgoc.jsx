...
const ProductList = () => {
  ...
  const filteredProducts = selectedidcategory == null ? products : products.filter(p => p.idcategory === selectedidcategory);
          ...
          <aside className="product-list-sidebar" aria-label="Lọc theo danh mục">
            ...
            <ul className="product-list-sidebar_list">
              <li>
                <button
                  type="button"
                  className={`product-list-sidebar_btn ${selectedidcategory == null ? "product-list-sidebar_btn--active" : ""}`}
                  onClick={() => setSelectedidcategory(null)}
                >
                  Tất cả
                </button>
              </li>
              {categories.map(cat => (
                <li key={cat.id}>
                  <button
                    type="button"
                    className={`product-list-sidebar_btn ${selectedidcategory === cat.id ? "product-list-sidebar_btn--active" : ""}`}
                    onClick={() => setSelectedidcategory(cat.id)}
                  >
                    {cat.name}
                  </button>
                </li>
        ...
        <div className="product-list-main">
          ...
          {filteredProducts.length > PRODUCTS_PER_PAGE && filteredProducts.length > 0 && (
            <div className="product-list-pagination" role="navigation" aria-label="Phân trang sản phẩm">
              <button type="button" className="product-list-pagination_btn" onClick={goPrev} disabled={safePage <= 1}>
                Trang trước
              </button>
              <span className="product-list-pagination_info">
                Trang {safePage} / {totalPages}
              </span>
              <button type="button" className="product-list-pagination_btn" onClick={goNext} disabled={safePage >= totalPages}>
                Trang sau
              </button>
            </div>
          )}
